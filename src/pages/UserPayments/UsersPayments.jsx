...
function UsersPayments() {
  ...
  useEffect(() => {
    dispatch(getUsersPayments(users.meter_number))
      .unwrap()
      .then((payload) => {
  ...
  function UpdateUserPayment(data) {
    ...
    dispatch(updateUsersPayment(new_data))
      .unwrap()
      .then((payload) => {
  ...
  function AddPayment(values) {
    ...
    setshowModalNew(false);
    setLoading(true);
    dispatch(addUserPayment(values))
      .unwrap()
      .then((payload) => {
        setLoading(false);
        Swal.fire("Record Updated", `${payload.message}`, "success");
  ...
  return (
    <div>
      ...
      {showModalNew ? (
        <Modal title="Add Payment Details" setshowModalNew={setshowModalNew}>
          {modalContent}
        </Modal>
      ) : null}
    </div>
