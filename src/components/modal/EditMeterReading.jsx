...
function EditMeterReading({existingreading ,handleMeterReadingEdit}) {

    const formik = useFormik({
        initialValues: {
            new_meter: existingreading,
        },
        validationSchema: Yup.object({
            new_meter: Yup.number().min(1).required("New meter reading required")
        }),
        onSubmit: (values) => {
            handleMeterReadingEdit(values,existingreading)
        }
    });
